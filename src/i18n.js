...
const KO = {
  ...
  titleLines: (day, dist, mobile) => `오늘의 길 ${day} · 궁궐까지 ${dist} m · 恐龍이 기다린다<br>${mobile ? '한 손이면 된다 — 누른 자리가 조이스틱, 기울여 조준, 누른 동안 발사' : '클릭한 곳으로 조준, 누른 동안 발사 · 방향키·Shift 도 된다 · 우클릭·Space 로 雷'}<br>雷 는 비격진천뢰 — 조준 링 자리에 떨어진다<br>장갑이 전부다 — 수리 상자를 쏘거나 들이받아라`,
  contHint: '누르면 코인 한 개',
...
const EN = {
  ...
  titleLines: (day, dist, mobile) => `TODAY'S ROAD ${day} · ${dist} m to the palace<br>${mobile ? 'one thumb is enough — touch anywhere for a joystick, tilt to aim, hold to fire' : 'click to aim, hold to fire · arrows + Shift work too · right-click / Space = 雷'}<br>雷 is the thunder bomb — it lands on the aim ring<br>armor is all you have — shoot or ram the repair crates`,
